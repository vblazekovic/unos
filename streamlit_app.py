...
# ---------------- Natjecanja i rezultati ----------------
    ...
    with st.form("comp_form"):
        ...
        with col2:
            # Dinamična kolona: omogućena samo kad je reprezentativni nastup
        ...
        # Treneri: iz baze + dodatni (auto-upis u bazu) — MULTISELECT
    ...
    if choice:
        ...
        with st.form("edit_comp_form"):
            ...
            with col2:
                rep_enabled_e = (kind == "REPREZENTATIVNI NASTUP")
                rep_sub = st.selectbox("Podvrsta (REP)", REP_SUB, disabled=not rep_enabled_e, key="rep_edit")
            with col3:
            ...
            # Treneri: MULTISELECT + dodatni (i auto-upis)
            coach_rows = conn.execute("SELECT full_name FROM coaches ORDER BY full_name").fetchall()
            coach_choices = [r[0] for r in coach_rows] if coach_rows else []
            preselected = [c for c in (coaches_e or "").split(", ") if c in coach_choices]
            c_coach_sel, c_coach_custom = st.columns([2,1])
            with c_coach_sel:
                coach_mult = st.multiselect("Trener(i) (iz baze)", coach_choices, default=preselected, key=f"coach_multi_{edit_id}")
            with c_coach_custom:
